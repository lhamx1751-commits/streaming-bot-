...
def _test_pg():
    ...
    try:
        ...
        use_ssl = not (url.hostname and "railway.internal" in url.hostname)
        conn = pg8000.native.Connection(
            host=url.hostname, port=url.port or 5432,
            database=url.path[1:], user=url.username,
            password=url.password, ssl_context=use_ssl, timeout=5
        )
        conn.close()
        return True
    except:
...
def _pg_conn():
    ...
    return pg8000.native.Connection(
        host=url.hostname, port=url.port or 5432,
        database=url.path[1:], user=url.username,
        password=url.password, ssl_context=use_ssl
    )

def _pg_run(conn, sql, params=()):
    """Convert $1,$2 style to :p1,:p2 for pg8000"""
    named_sql = re.sub(r'\$(\d+)', lambda m: f':p{m.group(1)}', sql)
    kwargs = {f'p{i+1}': v for i, v in enumerate(params)}
    return conn.run(named_sql, **kwargs)
...
def fetchone(sql, params=()):
    if USE_PG:
        ...
        conn = _sqlite_conn()
        try:
            r = conn.execute(sq, params).fetchone()
...
def fetchall(sql, params=()):
    if USE_PG:
        ...
        conn = _sqlite_conn()
        try:
            return [dict(r) for r in conn.execute(sq, params).fetchall()]
        finally:
            conn.close()
...
def insert(sql, params=()):
    if USE_PG:
        conn = _pg_conn()
        try:
            rows = _pg_run(conn, sql + " RETURNING id", params)
    ...
    if USE_PG:
        ...
        try:
            for t in [
                ...
                '''CREATE TABLE IF NOT EXISTS netflix_profil (
                    ...
                    nomor INTEGER NOT NULL, nama TEXT NOT NULL,
                    pin TEXT, expired TEXT, status TEXT DEFAULT 'aktif')''',
                '''CREATE TABLE IF NOT EXISTS disney (
                    id SERIAL PRIMARY KEY,
                    nama_paket TEXT DEFAULT 'DISNEY 1 BULAN SHARING',
                    no_hp TEXT, email TEXT NOT NULL,
                    expired_langganan TEXT NOT NULL, catatan TEXT,
                    status TEXT DEFAULT 'aktif',
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''',
                '''CREATE TABLE IF NOT EXISTS disney_perangkat (
                    id SERIAL PRIMARY KEY, disney_id INTEGER NOT NULL,
                    nama TEXT NOT NULL, expired TEXT,
                    status TEXT DEFAULT 'aktif')''',
                '''CREATE TABLE IF NOT EXISTS youtube (
                    id SERIAL PRIMARY KEY, email TEXT NOT NULL,
                    password TEXT, expired TEXT NOT NULL, catatan TEXT,
                    status TEXT DEFAULT 'aktif',
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''',
                '''CREATE TABLE IF NOT EXISTS pengingat_log (
                    id SERIAL PRIMARY KEY, tipe TEXT NOT NULL,
                    ref_id INTEGER NOT NULL, jenis TEXT NOT NULL,
        ...
        conn.executescript('''
            CREATE TABLE IF NOT EXISTS netflix (
                ...
                password TEXT, tgl_ubah_pw TEXT, metode_bayar TEXT,
                catatan TEXT, status TEXT DEFAULT 'aktif',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
            CREATE TABLE IF NOT EXISTS netflix_profil (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                netflix_id INTEGER NOT NULL, nomor INTEGER NOT NULL,
                nama TEXT NOT NULL, pin TEXT, expired TEXT,
                status TEXT DEFAULT 'aktif');
            CREATE TABLE IF NOT EXISTS disney (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                nama_paket TEXT DEFAULT 'DISNEY 1 BULAN SHARING',
            ...
            CREATE TABLE IF NOT EXISTS disney_perangkat (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                disney_id INTEGER NOT NULL, nama TEXT NOT NULL,
